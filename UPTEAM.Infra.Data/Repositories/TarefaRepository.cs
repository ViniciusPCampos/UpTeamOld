...
namespace UPTEAM.Infra.Data.Repositories
{
    public class TarefaRepository : RepositoryBase<tb_tarefa>, ITarefaRepository
    {
        ...
        {
            return Db.Set<tb_tarefa>().Where(x => x.cod_usuario == userId);
        }

        public IEnumerable<tb_tarefa> GetByProject(int projectId)
        {
            return Db.tb_tarefa
                .Join(Db.tb_sprint, tarefa => tarefa.cod_sprint, sprint => sprint.idt_sprint, (tarefa, sprint) => new { tb_tarefa = tarefa, tb_sprint = sprint })
                .Where(tarefaSprint => tarefaSprint.tb_sprint.cod_projeto == projectId)
                .Select(tarefaSprint => tarefaSprint.tb_tarefa);
        }

        public IEnumerable<tb_tarefa> GetBySprint(int sprintId)
        {
            return Db.Set<tb_tarefa>().Where(x => x.cod_sprint == sprintId);
        }
    }
}
